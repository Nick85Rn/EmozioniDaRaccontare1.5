          ...
          {books.map((book) => {
             ...
             const cardContent = (
              <div className="clay-card" style={{ background: '#fff', borderRadius: '20px', overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column' }}>
                <div style={{ height: '180px', background: bgColor, display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative' }}>
                  {imageSrc ? (
             ...
             return (
              <div key={book.id}>
                {book.is_premium ? (
                  <PremiumLock>
                    <Link to={`/ebook/${book.id}`} style={{ textDecoration: 'none' }}>{cardContent}</Link>
                  </PremiumLock>
                ) : (
                  <Link to={`/ebook/${book.id}`} style={{ textDecoration: 'none' }}>{cardContent}</Link>
                )}
