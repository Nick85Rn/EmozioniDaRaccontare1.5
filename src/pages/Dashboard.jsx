...
const Dashboard = () => {
  ...
  const fetchStories = async () => {
    try {
      // Scarica TUTTO (*) per prendere anche il campo 'pages' nascosto
  ...
  // FUNZIONE DETECTIVE: Trova l'immagine ovunque sia nascosta
  const getCover = (story) => {
    // 1. Cerca nella colonna principale
    if (story.image_url) return story.image_url;
    // 2. Cerca dentro la prima pagina (JSON) - FONDAMENTALE per le tue storie vecchie
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '25px' }}>
          {stories.map((story) => {
            ...
            // Definisci il contenuto della card
            const cardContent = (
            ...
            // Logica Link e Premium
            return (
              <div key={story.id} style={{ height: '100%' }}>
                {story.is_premium ? (
                  <PremiumLock>
                    <Link to={`/story/${story.id}`} style={{ textDecoration: 'none' }}>{cardContent}</Link>
                  </PremiumLock>
                ) : (
                  <Link to={`/story/${story.id}`} style={{ textDecoration: 'none' }}>{cardContent}</Link>
                )}
